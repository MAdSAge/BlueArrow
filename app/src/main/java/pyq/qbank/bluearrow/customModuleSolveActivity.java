...
public class customModuleSolveActivity extends AppCompatActivity implements pasueProgressBar {

    private static final int TIMER_DURATION = 54000;
    Box<entityChapter> red;
    private RecyclerView recyclerView;
    private customModuleSolveActivityAdapter adapter;
    private List<entityCustomModuleMcq> topicList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        red = boxStore.boxFor(entityChapter.class);
        ...
        chapterTitle = test_Data.getStringExtra("customModuleName");
        ...
        adapter = new customModuleSolveActivityAdapter(topicList, Mode, this);
    ...
    private void startTimer(long duration) {
        countDownTimer = new CountDownTimer(duration, 1000) {
            ...
            public void onFinish() {
                pbar.setProgress(0);
                Toast.makeText(customModuleSolveActivity.this, "Time is up!", Toast.LENGTH_SHORT).show();
                timeUP(recyclerView);
            }
        };
    ...
    private void timeUP(RecyclerView recyclerView) {
        ...
        if (layoutManager instanceof LinearLayoutManager) {
            ...
            if (position != RecyclerView.NO_POSITION) {
                ...
                entityCustomModuleMcq currentMcq = topicList.get(position);

                // Get the ViewHolder for the current position
                RecyclerView.ViewHolder viewHolder = recyclerView.findViewHolderForAdapterPosition(position);

                if (viewHolder instanceof mcq_model_adapter.mcqViewHolder) {
                    customModuleSolveActivityAdapter.mcqViewHolder currentHolder = (customModuleSolveActivityAdapter.mcqViewHolder) viewHolder;
    ...
    private List<entityCustomModuleMcq> getMcQs(String chapterTitle) {
        Box<entityCustomModuleMcq> mcqBox = boxStore.boxFor(entityCustomModuleMcq.class);
        List<entityCustomModuleMcq> mcqList = mcqBox.query(entityCustomModuleMcq_.customModuleName.equal(chapterTitle)).build().find();
    ...
    private void submitQuiz() {
        ...
        solveNextModule.setOnClickListener(v -> {
            ...
            Intent intent = new Intent(this, customModuleSelection.class);
        ...
        doReview.setOnClickListener(v -> {
            ...
            Intent intent = new Intent(this, customModule_qbankMcqReview.class);
            intent.putExtra("customModuleName", chapterTitle);
            startActivity(intent);
            finish();
    ...
    private void calculateScores(entityChapter currentChap) {
        ...
        for (entityCustomModuleMcq m : topicList) {
